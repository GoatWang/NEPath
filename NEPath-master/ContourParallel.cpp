...
paths ContourParallel::OffsetClipper(const double* x, const double* y, double dis, int length, bool wash/*=true*/, double washdis/*=0.5*/, int num_least/*=50*/) {
	...
	for (int i = 0; i < length; ++i) {
		// Normalize the point to have center as origin (0, 0)
...
// The algorithm works by: (ChatGPT)
// Creating a tree of pathnodes: It constructs an offset tree using root_offset where each node represents a toolpath generated by offsetting the previous path.
// Depth-first search (DFS): It performs a depth-first search through the tree to collect all toolpaths.
// Clearing memory: It clears each node's data after processing and sets its children to NULL before deleting the DFS stack and the root node.

// Parameters: (ChatGPT)
// contour: The outer boundary of the area to be machined.
// holes: A set of inner boundaries (holes) that should not be machined.
// dis: The spacing between successive toolpaths.
// wash: A boolean flag that, if true, resamples the output paths to ensure a uniform distance between waypoints.
// washdis: The maximum allowed distance between waypoints when wash is enabled.
// num_least: The minimum number of waypoints to ensure in the output.
...
paths ContourParallel::tool_compensate(const path& contour, const paths& holes, double dis, bool wash/*=true*/, double washdis/*=0.5*/, int num_least/*=50*/) {
	...
	Path Contour;
	for (int i = 0; i < contour.length; ++i) {
		// Conversion from double to int: The Clipper library primarily works with integer coordinates for numerical stability and to avoid floating-point precision issues. The function converts double coordinates to integers to use Clipper's functionality.
		Contour << IntPoint(double2cInt(contour.x[i], scale, delta_x), double2cInt(contour.y[i], scale, delta_y));
	}
	Contour << Contour[0];
