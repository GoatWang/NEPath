...
public:
	static cInt double2cInt(const double& d, double scale, double delta_pos = 0.0); // Transform from a double variable to a cInt variable
	static double cInt2double(const cInt& c, double scale, double delta_pos = 0.0); // Transform from a cInt variable to a double variable
...
private:
	...
	static bool path_subset(const Path& p1, const Path& p2); // Determine whether p1 is enclosed by p2
public:
